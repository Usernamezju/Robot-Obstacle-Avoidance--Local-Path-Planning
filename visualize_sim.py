...
def _can_use_gui() -> bool:
    """检查当前环境是否具备弹窗显示能力。"""
    return bool(matplotlib.get_backend()) and bool(__import__("os").environ.get("DISPLAY"))


def plot_episode(seed: int, save_path: str | None = None, show: bool = False) -> Path | None:
    """绘制单局轨迹，可保存图片并可选弹窗显示。"""
    cfg = SimConfig()
...
if __name__ == "__main__":
    ...
    plot_episode(seed=args.seed, save_path=output, show=args.show)
